...
import { inform, success } from '@/app/functions/notifyUser';
import OrderStatusSteppers from '@/app/MUIComponents/stepper/page';
...
export default function page() {
    ...
    let handleOrderCancel = () => {
        console.log('cancel order clicked');
        axios.get(`${backendBaseUrl}/order/update-shipment-status/${orderId}/5`)
            .then(res => res.data)
            .then(finalRes => {
                console.log(finalRes.message);
                setOrderDetails(finalRes.data);
                inform(toast, <h1>Order Cancelled Successfully</h1>);
    ...
    return (
        <div>
            <div className='mx-auto max-w-[1460px]'>
                <h1 className='font-bold text-[25px] mt-[30px]'>Order Details</h1>
                <h2 className='text-[20px] font-bold'>#{orderId} </h2>
                <ToastContainer/>
                {
                    orderDetails ?
                        <div className='p-[20px]'>
                            ...
                            {
                                orderDetails.shipmentStatus == 5 ?
                                    <></>
                                    :
                                    <div className='mx-auto my-[50px]'>
                                        <OrderStatusSteppers currentKey={orderDetails.shipmentStatus} />
                                    </div>
                            }
                            ...
                            <div className='flex justify-end'>
                                {
                                    orderDetails.shipmentStatus == 5 ?
                                        <h1><img src='/images/cancelled.png' className='w-[120px]' /></h1>
                                        :
                                        <button onClick={handleOrderCancel} className='text-red-500 font-bold'>Cancel Order</button>
                                }
